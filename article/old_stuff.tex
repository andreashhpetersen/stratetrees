...
% OLD DESCRIPTION OF QTREE TO DT CONVERSION
...
% ALTERNATE VERSION OF THE PSEUDO-CODE
...
\begin{algorithm}[!ht]
    \caption{MaxPartitions}\label{alg:MaxPartitionsElaborate}
    ...
    \begin{algorithmic}[1]
        ...
        \While{$\mathcal{T}_{track}$ has unexplored leaves}
            \State{%
                $\nu \gets$ find unexplored region in
                $\mathcal{T}_{track}$
            }

            \State{%
                $(p^{\min}, p^{\max}),\, a \gets$
                find region bounds and action of $\nu$ in $\mathcal{T}$
            }
            ...
            \While{%
                ...
                \If{%
                    $\nu'$ is explored in $\mathcal{T}_{track}$ or $a' \neq a$
                    for any $a' \in \mathcal{T}(\nu')$
                }
                    \State{$healing \gets false$}
                    \State{$\Delta_p \gets$ last legal value of $\Delta_p$}
                    \State{mark $d$ as exhausted}

                \ElsIf{%
                    $\nu'$ breaks any regions in $\mathcal{T}$
                }
                    \State{$candUpdate \gets$ value of largest region bound in}
                    \State{dimensoin $d$ among the broken regions}

                    \If{%
                        $candUpdate \leq \Delta_{p_{d}}$
                    }
                        \State{$healing \gets false$}
                        \State{$\Delta_p \gets$ last legal value of $\Delta_p$}
                        \State{mark $d$ as exhausted}
                    \Else
                        \State{$\Delta_{p_{d}} \gets candUpdate$}
                    \EndIf%
                \EndIf%
...
% OLD DESCRIPTIONS OF MINIMIZATION TECHNIQUES
