...
variable bounds $(u_{0,i}, l_{0,i})$ in $S_0$. We denote this operation
...
\begin{algorithm}[ht]
    ...
    \begin{algorithmic}[1]
        \Function{PutAtBranchNode}{$\texttt{Node}(V_k, b, high, low),
        ...
        \State{$\mathbf{return}\quad \texttt{Node}(V_k, b, high, low)$}
        \EndFunction%
...
\begin{algorithm}[h!]
    ...
    \begin{algorithmic}[1]
        \Procedure{Split}{$action, q, var, bound, state$}
            ...
            \State{$\mathbf{return}\quad \texttt{Node}(var, bound, high, low)$ }
            \item[]
        \EndProcedure%
    \end{algorithmic}
