...
\begin{algorithm}[!ht]
    ...
    \begin{algorithmic}[1]
        ...
        \While{$\mathcal{P}$ is not empty}
            ...
            \item\Comment{The list also need to be sorted according to distance
                to $p^{\min}$ }
            \For{$V_i, c$ in $\{(V_i, c) \mid (V_i, c) \in \mathcal{C},
                c > p^{\min}_i,  V_i \notin  exhausted \} $}
