...
\[
    ...
    \begin{cases}
        p & \text{if } |E|  =  K \\
        grow(p, E \cup \{i\}) &
            \text{if } |\mathcal{T}((s, s'))| > 1
            \text{ or $s'$ is explored} \\
        grow(p', E \cup \{i\}) & \text{if } p_i + 1 = M_i \\
        grow(p', E) & \text{otherwise}
    \end{cases}
\] 

\noindent where $grow$ is parametized by $\theta = (\mathcal{T}, M, \mathcal{C},
s)$ with $\mathcal{T}$ being a binary decision tree over the domain
$\mathbb{R}^K$ inducing a partitioning $\mathcal{A}_{\mathcal{T}}$, $M$ being a
$K$-dimensional vector of the number of constraints on each dimension $i$ in
$\mathcal{A}_{\mathcal{T}}$, $\mathcal{C}$ being an $K \times m$ matrix of the
sorted constraints, such that  $\mathcal{C}_{i,j}$ is the $j$th smallest
constraint on dimension $i$ for each $j = 1,2,\ldots,M_i$ and $s \in
\mathbb{R}^K$ is a point in the domain. Further, to avoid cluttering the
expression for $grow$, we use $i, s'$ and $p'$ defined as

\begin{align*}
    i &= \argmin_{i \in \{ i \mid i = 1,\ldots,K, i \notin E, p_i < M_i \}}
        \mathcal{C}_{i,p_i+1} - \mathcal{C}_{i,p_i} \\
    s' &= (\mathcal{C}_{i,p_1}, \ldots, \mathcal{C}_{i,p_i + 1},\ldots,
           \mathcal{C}_{i,p_K}) \\
    p' &= (p_1, \ldots, p_i + 1, \ldots, p_K)
\end{align*}
...
%     \begin{algorithmic}[1]
%         \Function{Grow}{$p, E$}
%             \If{$|E| = K$} \Return $p$ \EndIf%

%             \State{%
%                 $\displaystyle i \gets \argmin_{i \in \{
%                     i \mid i \notin  E, p_i < M_i, i = 1,\ldots, K
%                 \}}
%                     \mathcal{C}_{i,p_i + 1} - \mathcal{C}_{i,p_i}$
%             }
...
\begin{algorithm}[!ht]
    ...
    \begin{algorithmic}[1]
        ...
        \Require{%
            $\mathbf{0}^K$: A $K$-dimensional point with all elements being 0.
        }
        ...
        \While{$\mathcal{P}$ is not empty}
            ...
            \State{%
                $p^{\max} \gets grow(
                    p^{\max}, \{\} \mid (\mathcal{T}, M, \mathcal{C}, s^{\min})
                )$
            }
