...
\begin{figure}[ht]
    ...
    \caption{%
        ...
        In~\subref{fig:complexExampleTree} the decision tree representation is
        given for state dimensions $x$ and $y$ and possible actions $a$, $b$ and
        $c$. In~\subref{fig:complexExample2dVisual} a 2D visualization of the
...
\begin{algorithm}[ht]
    ...
    \begin{algorithmic}[1]
        \Function{MaxPartitions}{$\mathcal{T}$, $\mathcal{C}$, $\mathcal{V}$, $p_{start}$}
        ...
        \While{$\mathcal{P}$ is not empty}
            ...
            \State{\Call{Sort}{$\mathcal{P}$}}
            \State{$p^{\min} \gets \Call{Pop}{\mathcal{P}}$}

            \If{$p^{\min}$ has been visited}
                \State{continue}
            \EndIf%

            \State{$p^{\max} \gets p^{\min}$}
            \State{\Call{SortAccordingToDistance}{$\mathcal{C}$, $p^{\min}$}}

            \item
            \For{$V_i, c$ in $\mathcal{C}$}
                \If{$c <= p^{\min}_i$ or $V_i \in exhausted$}
                    \State{continue}
                \EndIf%

                \State{$oldVal \gets p^{\max}_i$}

                \State{$p^{\max}_i \gets c$}

                \item
                \If{%
                    $|\mathcal{T}((p^{\min},p^{\max}))|$ $>$ 1 \textbf{or}
                    $p^{\max}$ is explored \textbf{or} $p^{\max}_i = V^{\max}_i$ 
                }

                    \State{$exhausted \gets exhausted \cup V_i$}

                    \If{$|exhausted| < |\mathcal{V}|$}
                        \State{continue}
                    \EndIf%

                    \If{$p^{\max}_i < V^{\max}_i$}
                        \State{$p^{\max}_i \gets oldVal$}
                    \EndIf%

                    \State{$regions \gets regions \cup (p^{\min},p^{\max})$}

                    \item
                    \For{$V_i$ in $\mathcal{V}$}
                    \If{%
                        $p^{\min}_i \neq p^{\max}_i$
                        \textbf{and} $p^{\max}_i < V^{\max}_i$
                    }
                        \State{$p' \gets p^{\min}$}
                        \State{$p'_i \gets p^{\max}_i$}
...
\paragraph{} \textit{Example 1} As a running example, we consider an articifial
